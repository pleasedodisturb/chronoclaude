...
`src/config.js` exposes `isEnabled(key, env)` and a `SURFACES` map. Each user-facing surface is gated by a `CLAUDE_TIMING_*` env var. Most are **on by default**, disabled only by an explicit falsy value (`0`/`false`/`off`/`no`): `CLAUDE_TIMING_PASSIVE`, `CLAUDE_TIMING_IDLE_NOTE`, `CLAUDE_TIMING_MESSAGE_DISPLAY`, `CLAUDE_TIMING_TIMELINE`. The exception is the **opt-in** set in `OPT_IN_SURFACES` (currently just `stopTimestamp` / `CLAUDE_TIMING_STOP_TIMESTAMP`), which is **off by default** and turns on only for an explicit truthy value (`1`/`true`/`on`/`yes`) — the inverse default. `stopTimestamp` is the IDE-extension workaround: it makes the `Stop` hook emit a per-turn `[HH:MM:SS]` `systemMessage` for VS Code/JetBrains panels, where the inline `MessageDisplay` marker never fires; it's off by default so the terminal TUI (which already shows the inline marker) isn't double-stamped. Toggles gate only emitted **output** — `Stop`/`PreCompact` and state writes always run so the shared state stays coherent for whichever surfaces are on. The `/chronoclaude-config` command reports state and prints a paste-ready snippet.
...
2. `Stop` (`scripts/stop.js`) — sets `lastStopAt = now`, computes `lastTurnExecMs` from `lastUserPromptAt`. Only computes `lastTurnExecMs` if `session.lastStopAt` was previously falsy (avoids recompute on Stop re-fires within the same turn). When `CLAUDE_TIMING_STOP_TIMESTAMP` is enabled (opt-in, default off), also writes a `{ systemMessage: "[HH:MM:SS]" }` to stdout — the IDE-extension workaround surface; state is persisted unconditionally first, the toggle gates only the emit.
